...
:: ──────────────────────────────────────────────────────────────────────────
:: Самодостаточный скрипт деплоя Cloudflare Worker для Mirrly TG Proxy.
:: Не требует никаких внешних файлов — весь код воркера вшит внутрь.
...
if %errorlevel% neq 0 (
    echo.
    echo  [X] PowerShell не найден в системе.
    echo.
    pause
    exit /b 1
)

:: Запускаем PowerShell, который сам читает этот .bat и извлекает PS1-блок
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$batFile = '%~f0';" ^
    "$lines = [System.IO.File]::ReadAllLines($batFile, [System.Text.Encoding]::UTF8);" ^
    "$start = ($lines | Select-String -Pattern '^::PS1_BEGIN$' | Select-Object -First 1).LineNumber;" ^
    "$end   = ($lines | Select-String -Pattern '^::PS1_END$'   | Select-Object -First 1).LineNumber;" ^
    "if (-not $start -or -not $end) { Write-Host '[X] Блок PS1 не найден в .bat файле' -ForegroundColor Red; exit 1 };" ^
    "$ps1 = $lines[($start)..($end - 2)] -join \"`n\";" ^
    "$tmp = [System.IO.Path]::Combine([System.IO.Path]::GetTempPath(), 'mirrly_deploy_' + [System.Guid]::NewGuid().ToString('N') + '.ps1');" ^
    "[System.IO.File]::WriteAllText($tmp, $ps1, [System.Text.Encoding]::UTF8);" ^
    "try { & powershell -NoProfile -ExecutionPolicy Bypass -File $tmp } finally { Remove-Item $tmp -ErrorAction SilentlyContinue }"
...
# Файл истории сохраняется рядом с запущенным .bat (или в Documents, если нет прав)
