...
class ProxyStats {
    ...
    fun parseNativeStats(rawStr: String) {
        ...
        try {
            val connsMatch = Regex("""(?:active_connections|active_conns|active|conns|connections)[\s=:]+['"]?(\d+)""", RegexOption.IGNORE_CASE).find(rawStr)
            if (connsMatch != null) {
                val active = connsMatch.groupValues[1].toIntOrNull()
                if (active != null) activeConnections.set(active)
            }

            val rxMatch = Regex("""(?:rx_bytes|bytes_recv|download_bytes|bytes_received)[\s=:]+['"]?(\d+)""", RegexOption.IGNORE_CASE).find(rawStr)
            if (rxMatch != null) {
            ...
            val txMatch = Regex("""(?:tx_bytes|bytes_sent|upload_bytes)[\s=:]+['"]?(\d+)""", RegexOption.IGNORE_CASE).find(rawStr)
            if (txMatch != null) {
