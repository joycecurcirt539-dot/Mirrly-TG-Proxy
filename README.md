...
> [!IMPORTANT]
> **Приоритет личного Cloudflare Worker**:
> Воркер разработчика предназначен исключительно для первичной проверки связи сразу после установки («из коробки»). Для 100% конфиденциальности данных, независимости от чужих лимитов и максимальной пропускной способности SOCKS5 и MTProto **настоятельно рекомендуется развернуть собственный воркер** (бесплатно, за 2 минуты по встроенной инструкции).
...
### Зачем в приложении есть воркер разработчика?
Встроенный резервный воркер разработчика присутствует в приложении по трем причинам:
* **Мгновенный старт «из коробки» (Plug & Play)**: Новый пользователь может сразу после установки нажатием одной кнопки проверить работу приложения и убедиться в обходе блокировок без необходимости предварительно регистрироваться в Cloudflare.
* **Демонстрация технологии**: Показывает наглядную работоспособность WSS-туннелирования на практике.
* **Полное автоотключение при вводе своего домена**: Как только вы указываете свой персональный домен воркера в Настройках, тестовый воркер разработчика отключается на 100%, и весь трафик перенаправляется строго на ваш личный аккаунт. Исходный код воркера доступен в файле [`docs/cloudflare_worker.js`](docs/cloudflare_worker.js).
