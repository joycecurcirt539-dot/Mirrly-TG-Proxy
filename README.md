...
*Защита от ТСПУ, DPI-фильтрации, замедлений и блокировок IP-адресов дата-центров Telegram. Работает на нативном C/Rust ядре с пулом прогретых сокетов WsPool и поддержкой Cloudflare CDN.*
...
- **Обход блокировок IP DC1-DC5**: Провайдеры блокируют прямые IP-адреса Telegram. Прокси направляет трафик к серверам Cloudflare, обходя блокировку целевых IP.
- **Защита от DPI и ТСПУ**: Трафик маскируется под обычные зашифрованные HTTPS WebSocket-сессии (WSS). Для фильтров провайдера данный трафик неотличим от посещения защищенного веб-сайта.
- **Без VPN-сервиса**: Работает строго локально на `127.0.0.1:1443`. Не перехватывает трафик других приложений и не расходует лишний заряд батареи.
...
- **100% Открытый код**: Приложение не собирает персональные данные, сообщения или историю посещений. Работает строго локально.
