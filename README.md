...
> Резервные воркеры разработчика встроены исключительно как демонстрационный пример технологии для быстрой проверки работы сразу после установки («из коробки»). Для 100% конфиденциальности данных, независимости от чужих лимитов и максимальной пропускной способности SOCKS5 и MTProto **настоятельно рекомендуется развернуть собственный воркер** (бесплатно, за 2 минуты по встроенной инструкции).
...
### Тестовые воркеры разработчика и кастомные домены:
В приложение встроены тестовые воркеры разработчика Mirrly для первичной проверки функционала. Введенный пользователем персональный домен воркера имеет абсолютный приоритет для протоколов MTProto и SOCKS5 — при его заполнении тестовые воркеры отключаются полностью. Исходный код воркера доступен в репозитории: [`docs/cloudflare_worker.js`](docs/cloudflare_worker.js).
