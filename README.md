...
6. *(Опционально)* Если воркер был создан со старыми настройками: перейдите в **Settings** → **Runtime** и убедитесь, что **Compatibility Date** установлена не ранее `2023-05-18` и включен флаг **Node.js compatibility** (`nodejs_compat`).
7. Скопируйте полученный адрес воркера (например: `my-tg-proxy.username.workers.dev`).
8. Откройте **Mirrly TG Proxy** → перейдите в **Менеджер воркеров** → нажмите **«Добавить воркер»** и укажите ваш адрес.
...
* **100 000 бесплатных запросов в сутки**: Каждый бесплатный аккаунт Cloudflare предоставляет 100 000 бесплатных обращений ежедневно. При необходимости распределения нагрузки можно создать несколько воркеров или использовать разные аккаунты.
